...
2.1.4. **동적 슬롯 이름 (Dynamic Slot Names)**:
...
<!-- 사용 예시 -->
<DynamicSlot>
  <template #[slotName]> 동적 슬롯 내용 </template>
</DynamicSlot>
```
...
<FancyList>
  <template #default="{ item, isSelected }">
    <div :class="{ selected: isSelected }">{{ item.text }}</div>
  </template>
</FancyList>
```

2.1.6. **슬롯 폴백 (Fallback) 내용**:
...
<script setup>
  defineProps({
    items: {
      ...
      required: true,
      validator: (value) =>
        value.every((item) => "id" in item && "name" in item),
    },
    selectedId: {
      type: Number,
      default: null,
